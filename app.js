...
const Header = ({ nick, onSearch, currentView, onViewChange, onProfileClick }) => (
  React.createElement('header', {
    className: "sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-[#0a0a0acc] bg-[#0a0a0acc] border-b border-white/5"
  }, [
    React.createElement('div', {
      key: 'container',
      className: "mx-auto max-w-4xl px-4 py-3 flex items-center gap-3 justify-between"
    }, [
      React.createElement('div', {
        key: 'logo',
        ...
        React.createElement('h1', {
          key: 'title',
          className: "text-lg font-bold text-[--glow] cursor-pointer",
...
const Modal = ({ open, onClose, children }) => {
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', {
      key: 'content',
      className: "absolute inset-x-0 bottom-0 md:inset-0 md:m-auto md:h-[80vh] max-h-[90vh] overflow-auto mx-2 md:mx-auto md:max-w-2xl rounded-3xl bg-[--panel] border border-white/10 p-4"
    }, children)
  ]);
};
...
const PostCard = ({ post, onQuote, onOpen, onLike, onBookmark, onDelete, onReport, onAuthorClick, isLiked, isBookmarked, currentUser, isOwnPost }) => {
  ...
  return React.createElement('article', {
    className: "rounded-lg border border-white/5 bg-[--panel] hover:bg-[#1a1a1a] transition-colors"
  }, [
    React.createElement('div', {
      key: 'content',
      className: "p-4"
    ...
    React.createElement('div', {
      key: 'actions',
      className: "flex items-center justify-between border-t border-white/5 px-4 py-2 text-[--sub] text-sm"
    }, [
      React.createElement('div', {
        key: 'actionButtons',
...
const Composer = ({ nick, quoteTarget, onCancelQuote, onPost }) => {
  ...
  return React.createElement('div', {
    className: "rounded-lg border border-white/5 bg-[--panel] p-4"
  }, [
    React.createElement('div', {
      key: 'composerContent',
  ...
  return React.createElement('div', {
    ...
    React.createElement('main', {
      ...
      currentView === 'home' && React.createElement('div', {
        key: 'settings',
        className: "flex items-center justify-between text-xs text-[--sub] mb-6 p-3 rounded-lg bg-[--panel] border border-white/5"
      }, [
        React.createElement('div', {
          key: 'autoReply',
